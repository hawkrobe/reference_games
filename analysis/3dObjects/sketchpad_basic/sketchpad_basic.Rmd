...
d <- raw %>% 
  ...
  mutate(sketchLabel = sprintf('%s_%s', floor((row_number() - 1) / 32), trialNum)) %>%
  mutate(renderLabel = sprintf("%s_%04d", object1Name, pose)) %>%
  select(condition, object1Name, object2Name, object3Name, object4Name, sketchLabel, renderLabel) 
