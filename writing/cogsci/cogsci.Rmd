...
print(xtable(topWords, label = 'tab:words', caption = 'Top 10 unigrams and bigrams with the highest reduction'), 
      floating.environment = "table*", comment=F)
```
...
A final key signature of conventions is their *arbitrariness* [@Lewis69_Convention]: different pairs may converge on different but equally successful referring expressions. By the final round, for example, one pair successfully communicated about one tangram as the 'dancer' while another successfully used 'ice skater' to refer to the same object. However, once a pair adopts a convention, it is usually in their best interest to continue using it: in this sense, conventions are *path-dependent*. In our data, we can operationalize these concepts by comparing the entropy of the word distribution *across* pairs against the entropy *within* pairs. If referring expressions were *not* arbitrary in our task, we would expect the entropy to be very low across pairs, since all players would adopt the single best solution. If it were *not* path-dependent, then we would similarly expect the entropy to be higher within pairs, since players would use different words every round. \textcolor{red}{TODO: figure out how to present these numbers; probably not with figure, since we're collapsing across tangrams? Bar plot seems silly...}
