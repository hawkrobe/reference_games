...
As in several other recent applications of RSA [@GrafEtAl16_BasicLevel], we take meanings as mappings from utterance-state pairs to the interval $[0,1]$. These real-valued scores represent the extent to which the utterance applies to the state, and are normalized to sum to one for a given utterance: 
...
where $\{s_i, u_i\}$ is a set of observations of states $s_i$ and $u_i$ coming from previous exchanges. \textcolor{red}{TODO: this notation makes the subscripts when describing the space of states/utterances confusing}. The speaker then marginalize over this posterior distribution when reasoning about the listener, giving rise to the form of the pragmatic listener model we use throughout our model results (only going up to $n = 1$ in our recusion for simplicity):
...
Consider an environment with two abstract shapes ($\{s_1, s_2\}$), where the speaker must choose between two utterances ($\{u_1, u_2\}$) with equal cost. Their prior $P(\mathcal{L})$ over the meaning of each utterance is given by a Dirichlet distribution, so on the first round both utterances are equally likely to apply to either shape. If the speaker was trying to get their partner to pick $s_1$, then, they would assign .5 probability to both utterances, randomly sample one (say, $u_1$), and observe the listener's selection of a shape (say, $s_1$).

On the second round, the speaker uses the  observed pair $\{u_1, s_1\}$ to update their posterior over lexicons. Assuming a knowledgeable listener, lexicons in which the chosen utterance corresponds to the chosen shape become more likely, so the likelihood of producing that utterance to communicate $s_1$ again rises to 0.67. Because they learn about the lexicon by reasoning about observations coming from a pragmatic listener, they also have beliefs about the meanings of unheard utterances. If the agent must communicate $s_2$, their likelihood of producing $u_2$ has also risen to 0.67. Thus, the initial symmetry between the meanings can be broken by an initial random choice, leading to completely arbitrary but stable mappings. 
...
```{r modelResult, fig.env = "figure", fig.pos = "t", fig.align='left', fig.width=1, fig.height=1, set.cap.width=T, num.cols.cap=1, fig.cap = "\\label{fig:modelResult} Example trial"}
img1 <- png::readPNG("figs/2ae683.png")
grid::grid.raster(img1)
```

```{r modelResult2, fig.env = "figure", fig.pos = "t", fig.align='right', fig.width=1, fig.height=1, set.cap.width=T, num.cols.cap=1, fig.cap = "\\label{fig:modelResult} Example trial"}
